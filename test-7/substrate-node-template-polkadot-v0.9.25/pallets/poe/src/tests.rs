...
fn claim_transfer(){
	new_test_ext().execute_with(||{
		...
		let bounded_claim = BoundedVec::<u8,<Test as Config >::MaxClinetLenght>::try_from(claim.clone()).unwrap();
		assert_eq!( Some((2,frame_system::Pallet::<Test>::block_number())),  Proofs::<Test>::get(&bounded_claim) );
	})
}
...
fn claim_transfer_not_exist(){
	new_test_ext().execute_with(||{
		let claim = vec![0,1];
		assert_noop!(PoeModule::transfer_claim(Origin::signed(2), claim.clone(), 2),Error::<Test>::ClaimNotExist);
	})	
}
